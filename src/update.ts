...
const fileLocation = 'dist/domains.json';
...
axios.get('https://f.hubspotusercontent40.net/hubfs/2832391/Marketing/Lead-Capture/free-domains-1.csv')
  .then(response => {
    ...
    const curHash = hash(JSON.parse(fs.readFileSync(fileLocation).toString()));
    const newHash = hash(data);

    if (curHash !== newHash) {
      fs.writeFileSync('dist/domains.json', JSON.stringify(data));
